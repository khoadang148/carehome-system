...
export default function PhotoGalleryPage() {
  ...
  const getPhotoUrl = (photo: any) => {
    ...
    // Thử endpoint API trước
    const apiUrl = `${photosAPI.getPhotoUrl(photo.file_path)}?v=${version}`;
    
    // Fallback: thử truy cập trực tiếp file nếu API không hoạt động
    const fallbackUrl = `${process.env.NEXT_PUBLIC_API_URL || 'https://sep490-be-xniz.onrender.com'}/${photo.file_path}?v=${version}`;
