...
// Lightweight in-memory cache for GET requests to speed up production loads
// Applies to browser runtime only; respects Authorization token in cache key
type CachedResponse = {
  data: any;
  status: number;
  statusText: string;
...
const GET_CACHE_TTL_MS = 2 * 60 * 1000; // 2 minutes
const getCache: Map<string, CachedResponse> = new Map();
const pendingRequests: Map<string, Promise<any>> = new Map();

function buildCacheKey(config: any): string {
  try {
    const url = new URL((config.baseURL || '') + (config.url || ''), 'http://dummy');
    ...
    return `${url.pathname}?${query}|t=${token}`;
  } catch {
    return `${config.baseURL || ''}${config.url || ''}|t=${(config.headers && (config.headers['Authorization'] || config.headers['authorization'])) || ''}`;
  }
}
...
apiClient.interceptors.request.use(
  (config) => {
    ...
    // GET response caching/deduplication
    const method = (config.method || 'get').toLowerCase();
    const useCache = (config as any).useCache !== false && method === 'get';
    if (useCache) {
      const key = buildCacheKey(config);
      const cached = getCache.get(key);
      if (cached && Date.now() - cached.timestamp < GET_CACHE_TTL_MS) {
        // Serve from cache via custom adapter
      ...
      if (pending) {
        (config as any).adapter = async () => pending;
        return config;
      }
      // Wrap the default adapter to record the pending promise
      const defaultAdapter = ((config as any).adapter || (axios.defaults && (axios.defaults as any).adapter)) as any;
      if (typeof defaultAdapter === 'function') {
        (config as any).adapter = async () => {
          const p = defaultAdapter(config) as Promise<any>;
...
apiClient.interceptors.response.use((response) => {
  try {
    ...
    const useCache = (response.config as any).useCache !== false && method === 'get';
    if (useCache) {
