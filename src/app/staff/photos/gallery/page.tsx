...
export default function PhotoGalleryPage() {
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      console.log('Fetching rooms for photos:', photos.length);
      // Lấy danh sách id thực tế (dù là object hay string)
      ...
      console.log('Resident IDs to fetch rooms for:', residentIds);
      
      for (const residentId of residentIds) {
        if (!residentId || roomNumbers[residentId]) {
          console.log('Skipping residentId:', residentId, 'already has room or invalid');
          continue;
        }
        try {
          // Ưu tiên sử dụng bedAssignmentsAPI
          try {
            console.log('Fetching bed assignments for residentId:', residentId);
            const bedAssignments = await bedAssignmentsAPI.getByResidentId(residentId);
            console.log('Bed assignments for', residentId, ':', bedAssignments);
            ...
            console.log('Fetching care plans for residentId:', residentId);
            const assignments = await carePlansAPI.getByResidentId(residentId);
            console.log('Care plan assignments for', residentId, ':', assignments);
  ...
  // Debug: Log photos and roomNumbers
  useEffect(() => {
  ...
  const filteredPhotos = photos.filter(photo => {
    ...
    // So sánh resident_id đúng cách (có thể là object hoặc string)
  ...
  // Hàm lấy tên resident từ resident_id (hỗ trợ cả object và string)
  ...
  // Hàm lấy tên staff từ uploaded_by (hỗ trợ cả object và string)
